...
| S5 | **CORS strict** | ✅ Current (`C2-5`) | `app.cors.*` → `CorsProperties` + `corsConfigurationSource()` trong `SecurityConfig`; origin qua `CORS_ALLOWED_ORIGINS`. 🔴 **Trước `C2-5` repo có 0 dòng cấu hình CORS** — `CorsFilter` vẫn nằm trong chain (default của Spring Security) nên wiring *trông như* có, mà **không origin nào được phép** ⇒ FE khác origin không gọi được endpoint nào. `"*"` bị **ném lỗi lúc khởi động**, không phải khuyến cáo: API này gửi `Authorization` mọi request và CORS spec cấm wildcard trên credentialed request. Phải wire qua `http.cors(...)`, **không** phải `WebMvcConfigurer` riêng — preflight `OPTIONS` không mang token nên chỉ đường đó mới short-circuit trước `.anyRequest().authenticated()`. Test: `CorsConfigurationTest` (6 case) |
...
| S8 | **Audit logging** | ✅ Current (track `AR-*`, 2026-09-02) | 3 bảng bất biến (`audit_logs`, `audit_log_changes`, `audit_log_entities`) + hàng đợi `audit_outbox`. Ghi qua **transactional outbox**: outbox row nằm **trong** transaction nghiệp vụ nên business commit ⇒ event chắc chắn còn, business rollback ⇒ không thể có `SUCCESS` giả; event `FAILURE` ghi ở `REQUIRES_NEW` nên sống sót qua rollback. 🔴 **Trước đó pipeline là `@TransactionalEventListener(AFTER_COMMIT)`, chỉ giao event khi publisher đang trong transaction — mà `AuthService` không có `@Transactional` ⇒ MỌI event auth bị vứt bỏ im lặng.** `V68` chặn UPDATE/DELETE ở tầng DB cho mọi role. ⚠️ Gọi đúng là **"append-only activity trail"**, **không** gọi "tamper-proof": `payload_hash` đã lưu nhưng chưa có anchor ngoài DB. Chi tiết: `common/audit/CLAUDE.md`
...
| S15 | **Audit input hardening** | ✅ Current (`AR-1`) | `AuditInputSanitizer` là **boundary duy nhất**, áp ở `AuditRecorder` cho cả draft nên producer mới không thể quên. 🔴 **Trước đó `X-Trace-Id` (filter nhận 64 ký tự) đổ vào cột `VARCHAR(32)` ⇒ client tự tắt được audit trail của chính mình bằng một header** — insert chết *sau khi* business đã commit. Cắt trên ranh giới code point (cắt giữa surrogate pair ⇒ Postgres từ chối `invalid byte sequence for encoding "UTF8"`, đổi lỗi insert này lấy lỗi insert khác); JSON quá cỡ **thay bằng marker + sha256**, không cắt (nửa document JSON không phải JSON, mà cột là `jsonb`); **không bao giờ** lưu `Throwable.getMessage()` thô — dùng `reasonCode` = `ErrorCode.code()`; field nhạy cảm khai bằng `@AuditSensitive`, denylist theo tên chỉ là lưới đỡ |
...
| A2 | **Idempotency** | POST nhạy cảm → `Idempotency-Key` header, **lưu ở cột trên chính document row + UNIQUE constraint** (không phải Redis, không TTL). Đang có: `stock_movements` (`(key, movement_type)`, `V37`/`B69`), `goods_receipts` (`(purchase_order_id, key)`, `V20`), `material_issues` / `production_receipts` / `production_executions` (UNIQUE toàn bảng), **`mrp_runs` (`V58`, thêm 2026-08-14)**. Replay = trả lại document cũ; replay **khác payload** ⇒ `409 IDEMPOTENCY_CONFLICT` (`IdempotencySupport.ensureSamePayload`). 🔴 Hàng này trước 2026-08-14 ghi "MRP run → lưu Redis TTL 24h" — **sai hai lần**: `POST /planning-runs` chưa hề đọc header (FE gửi 1 key 3 lần ⇒ 3 run, xem `CLAUDE.md §0.44`), và repo **chưa bao giờ** lưu idempotency ở Redis |
...
| T2 | **Testcontainers** | ✅ Current | `AbstractPostgresIntegrationTest` (Singleton Container, `postgres:16-alpine`) + **22** `*IT.java` (2026-09-02; nguồn chính xác vẫn là `find src/test -name "*IT.java"`) (số class cập nhật 2026-08-14; danh sách liệt kê dưới đây dừng ở 11 và **chưa** kể `UomRepositoryIT`, `WorkOrderOperationRepositoryIT`, `AuditLogRepositoryIT`, `SalesOrderUpdateLinesIT`, `RolePermissionRepositoryIT`, `StockMovementRepositoryIT`, `WorkCalendarLookupServiceIT` — nguồn chính xác là `find src/test -name "*IT.java"`) (`FlywayMigrationIT`, `UserRoleAssignmentRepositoryIT`, `StockBalanceRepositoryIT`, `SalesOrderLineRepositoryIT`, **`ProductionFlowE2EIT`** từ `D1`, **`PurchaseOrderRepositoryIT`** từ `D4`, **`WorkOrderRepositoryIT`** từ `F7`, **`ProductionReceiptRepositoryIT`** + **`MaterialIssueRepositoryIT`** từ `F8`, **`MaterialReservationRepositoryIT`** từ `F9`, **`SupplierRepositoryIT`** từ bugfix 2026-08-04), chạy qua `mvn verify` (T4). **Khi nào bắt buộc dùng:** logic nằm trong JPQL (`R7`) — `F7` phải tạo `WorkOrderRepositoryIT` vì bộ lọc `B75` sống trong query; `F8` phải tạo 2 class nữa vì `B76` và 2 query list phẳng cũng vậy; `F9` thêm 1 class vì vế `status = ACTIVE` của query reservation batch cũng chỉ sống trong JPQL. Mock repository ở những chỗ đó chỉ tạo ra tautology. **Cũng là nơi duy nhất kiểm được `@Version`** — `WorkOrderRepositoryIT.concurrentGoodQuantityUpdate_*` (spec §10.3, nợ A đóng ở `F8`). 🔴 **Và là nơi duy nhất bắt được lỗi *type binding* của JPQL:** parameter `String` nullable đi vào `concat`/`like` mà thiếu `cast(... as string)` làm **cả endpoint 500** (`lower(bytea)`) — mock repository lẫn coverage đều báo xanh, xem `CLAUDE.md §0.24`. `SupplierRepositoryIT` là guard cho đúng chuyện đó.<br>🔴 **[2026-08-14] Cũng là nơi duy nhất bắt được lỗi `@EntityGraph` join-fetch nhiều bag.** `WorkCalendarRepository` fetch cùng lúc `weeklyShifts` và `Shift.breaks` ⇒ **mọi** `POST /work-orders/{id}/release` qua lịch có ca kèm giờ nghỉ trả **500**, mà toàn bộ 1025 case unit vẫn xanh vì mock repository không bao giờ dựng câu query. Điều làm nó sống sót lâu **không phải** thiếu `*IT` mà là **dữ liệu test quá sạch**: các `*IT` có sẵn đều có `Shift` nhưng không ca nào có break, nên bag thứ hai chưa từng bị chạm. ⇒ Khi viết `*IT` cho một aggregate, dựng fixture có **các collection lồng nhau được điền dữ liệu**, đừng chỉ điền entity gốc. Guard: `WorkCalendarLookupServiceIT`, xem `CLAUDE.md §0.45` |
...
| T6 | **Security test** | ✅ Current (deny+allow+verify+WithMockUser) | `*MethodSecurityTest` có đủ deny + `verify(guard)` khoá `PERM_*` + nhánh allow (T1); `PermissionCatalogTest` chống lệch code↔seed. `@WithMockUser` nay đã dùng ở `GlobalExceptionHandlerTest` + `AuthControllerTest` (T2).<br>🔴 **[`C2-5`] `PermissionCatalogTest` có một điểm mù đã từng nuôi một defect thật:** nó chứng minh mọi `PERM_*` trong `@PreAuthorize` **có dòng trong bảng `permissions`** — nhưng "tồn tại" khác "được cấp cho role nào", và sự thật thứ hai chỉ nằm ở `role_permissions`. 12 permission lõi từng chỉ `ADMIN` có ⇒ mọi account khác `admin` bị 403 ở màn hình lõi, mà toàn bộ suite vẫn xanh. Ma trận grant nay được canh ở `FlywayMigrationIT.migrate_v41_*` (3 case, **DB thật**), viết dưới cả hai dạng: **dương** (MANAGER phải có gì) và **cấm** (OPERATOR không được có gì — separation of duties). Thêm permission mới ⇒ phải nghĩ tới role, không chỉ tới `@PreAuthorize` |

> 🔴 **[2026-09-02, track `AR-*`] Lần thứ bảy `R7` được chứng minh, và là lần đắt nhất.** Ba defect
> của hạ tầng audit — mọi event auth bị vứt bỏ, event `FAILURE` mất khi rollback, một header dài làm
> hỏng insert sau khi business đã commit — sống sót qua **1184 case unit + 142 case IT đều xanh**.
> Lý do: chúng là phát biểu về **commit/rollback thật**, **unique index**, **trigger** và **độ rộng
> cột** — không thứ nào tồn tại trong một mock repository, và `@WebMvcTest` thì không nạp cấu hình
> transaction. `AuditPipelineIT` là guard.
>
> ⚠️ **Test outbox/commit phải tắt transaction bọc của `@DataJpaTest`**
> (`@Transactional(propagation = NOT_SUPPORTED)` + `TransactionTemplate` tường minh). `@DataJpaTest`
> mặc định rollback cuối mỗi test — thứ đang cần kiểm chính là *cái gì sống sót qua commit thật*, nên
> để nguyên thì mọi assertion đều vô nghĩa mà vẫn xanh.
>
