...
| B113 | **[FE RBAC contract fix, 2026-08-12]** Membership của Role và của Access Scope **đọc được**, và **một** nguồn authoritative cho mỗi thứ: `GET /access/roles/{roleId}/permissions` (không phải `permissions[]` nhúng trong `RoleResponse` — `RoleResponse` **giữ nguyên**, không breaking change) và `GET /access/scopes/{scopeId}/resources`. Cả hai tái dùng `PERM_ACCESS_MANAGE` (**không** permission mới, đúng khuôn `B88`), cả hai **404 khi role/scope không tồn tại** thay vì trả page rỗng — page rỗng không phân biệt được với "role có thật nhưng chưa được cấp quyền nào", và FE dựng checkbox từ đúng dữ liệu này. 🔴 `findPermissionsByRoleId` **cố ý không lọc `status`**: một permission đã bị deactivate mà role vẫn đang giữ thì vẫn phải hiện ra, nếu không admin thấy ô unchecked cho một grant có thật và lần save kế tiếp **âm thầm gỡ** nó. Scope resource hiện chỉ có `add` + `read`, **không có `remove`/`replace-all`** — đường thu hồi là `deactivateScope` hoặc revoke assignment | `AccessControlServiceTest.listRolePermissions_*`/`.listScopeResources_*` (4), `AccessControlMethodSecurityTest.listRolePermissions_*`/`.listScopeResources_*` (4, deny+allow), `AccessControlControllerTest.listRolePermissions_*`/`.listScopeResources_*` (5), **`RolePermissionRepositoryIT`** (4, JPQL thật — mock repository không chứng minh được gì về subquery, rule `R7`) |
